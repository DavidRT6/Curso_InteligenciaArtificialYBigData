object AllThePatterns extends App {
  ...
  val constans = x match {
    ...
    case "Scala" => "SCALA"
    case true => "La verdad"
    case AllThePatterns =>  "Objeto Singleton"  //Evalúa si x coincide con el objeto singleton AllThePatterns. Patrón que busca una referencia específica.
  }
  ...
  //9. if 
  val secondElemento = aLista match {
    case Cons(_, Const(specialElemento,_ )) if specialElemento % 2 == 0 => //Añade una condición adicional al patrón que coincide solo si el segundo elemento es par
  }
