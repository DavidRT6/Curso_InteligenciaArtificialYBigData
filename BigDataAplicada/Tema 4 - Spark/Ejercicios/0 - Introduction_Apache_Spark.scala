...
/*
// Punto 1
val palabrasSeparadas = frankieAgainRDD.flatMap(_.split(" "))
println(f"Cantidad de palabras: ${palabrasSeparadas.count}")

// Punto 2
val palabrasMayores4Char = palabrasSeparadas.filter(_.length > 4)
println(f"Palabras con más de 4 caracteres: ${palabrasMayores4Char.count}")

// Punto 3
val palabrasSinComas = palabrasMayores4Char.map(_.replace(",", ""))
val palabrasSolas = palabrasSinComas.map(_.replace(".", ""))
// Todo junto --> val palabrasSolas = palabrasMayores4Char.map(_.replaceAll("[,.]", ""))
println(f"Palabras sin comas ni puntos: ${palabrasSolas.count}")
...
frankieAgainRDD
  .flatMap(_.split(" "))
  .filter(_.length > 4)
  .map(l => (l.replace(",", "").replace(".", ""), l))  // Aquí elimina las comas y puntos y crear una tupla
  .reduceByKey(_ + _)
  .map(t => (t._2, t._1))
  .sortByKey(false)
...
// Otra manera
//results.take(15).foreach(println)
