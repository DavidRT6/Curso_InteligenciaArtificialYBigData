...
/*
val windowByMaritalSortedByBalance = Window
  .partitionBy("marital")
  .orderBy(desc("balance"), asc("age"))

val topThreePerMaritalDF = bankDF
  .withColumn("balance_order_for_the_marital", row_number().over(windowByMaritalSortedByBalance))
  .filter(col("balance_order_for_the_marital") <= 3)
  .select("marital", "job", "age", "balance")
  .orderBy("marital", "balance")
*/
...
val costOfVehiclesDF = vehiclesDF
  .filter(!col("cost_in_credits").isin("NA", "none"))
  .groupBy($"vehicle_class")
  .agg(min($"cost_in_credits").as("min_cost"))
  .agg(avg($"cost_in_credits").as("cost_difference"))
*/

val costOfVehiclesDF = vehiclesDF
  .filter(!col("cost_in_credits").isin("NA", "none"))
  .withColumn("cost_in_credits", col("cost_in_credits").cast(IntegerType))
  .withColumn("min_cost", min("cost_in_credits").over(Window.partitionBy("vehicle_class").orderBy("cost_in_credits")))
  ...
  .select(
    ...
    col("min_cost"),
    col("cost_difference"),
    //vehiclesDF.columns.filterNot(c => Seq("vehicle_class", "cost_in_credits", "min_cost", "cost_difference").contains(c)).map(col): _*
  )
