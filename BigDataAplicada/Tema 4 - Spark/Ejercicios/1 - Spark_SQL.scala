...
val charactersEyeDF = charactersDF
  .filter(!col("eye_color").isin("NA", "none"))
  .groupBy($"eye_color")
  .count() 
  .orderBy(desc("count"))
  .limit(1)
...
val netflixMoviesDF = netflixDF.filter(col("type") === "Movie")  //Versión Sonia
/* Mi versión
  .filter(!$"type".isin("TV Show"))
...
netflixMoviesDF  // Con mi versión: netflixDF
...
/*
val getYearAdded = udf((s: String) => s.split(","))

netflixMoviesDF
  .getYearAdded($"date_added")
  .show(10, false)
*/
